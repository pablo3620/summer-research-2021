...
petrol_series = ts(fuel_trade$Petrol[fuel_trade$year < 2020], frequency = 4, start = min(fuel_trade$year))
decomp_petrol = decompose(petrol_series, "multiplicative")

Time = attributes(petrol_series)$tsp
Time = seq(Time[1],Time[2], length.out=round((Time[2]-Time[1])*Time[3]+1))

# Convert td to data frame
dat = cbind(Time, with(decomp_petrol, data.frame(Observed=x, Trend=trend, Seasonal=seasonal, Random=random)))

ggplot(gather(dat, component, value, -Time), aes(Time, value)) +
  facet_grid(component ~ ., scales="free_y") +
  geom_line() +
  labs(y="Fuel usage (L)", x="Year") +
  ggtitle("Decomposition of multiplicative Petrol usage time series")+
  theme(plot.title=element_text(hjust=0.5))+
  theme_bw()
```
