...
This projects aims to understand how current seasonal trends in driving behaviour coupled with seasonal differences in the driving efficiency of elecric vehicles (EVs) will impact New Zealand's electricity grid under a future scenario where light vehicles are largely electrified. 

## Methodology

A variety of data sources were used. Distance traveled and vehicle efficiency (km/kWh) by month, as well as the region of the vehicle was collected from the onboard computers of `r length(unique(EV_data$vehicle))` vehicles between `r min(EV_data$year)` and `r max(EV_data$year)` as part of the 'Flip the Fleet' project.
