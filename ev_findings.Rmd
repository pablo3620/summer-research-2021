...
```{r}
eff_h_c_lm = lm((1/efficiency) ~ CDD + HDD + weather_region + model, data = EV_data[year >=2017,], na.action=na.omit)
summary(eff_h_c_lm)
```
...
#plot(eff_lm)
#plot(eff_reg_lm)
plot(eff_h_c_lm, col = c(1,2)[1+(model == "Tesla Model-X" | model == "Conversion to EV")])
```
