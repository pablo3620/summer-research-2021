...
#bad data vehicles
attach(EV_data)
EV_data = EV_data[vehicle != "38f81643" & vehicle != "d6082525" & vehicle != "ae05b6a6" & vehicle != "667822bd" & vehicle != "ad7a2a1e", ]
...
```{r}
eff_reg_lm = lm(efficiency ~ CDD + HDD + weather_region + model, data = EV_data, na.action=na.omit)
summary(eff_reg_lm)
```
...
```{r}
eff_h_c_lm = lm((1/efficiency) ~ CDD + HDD + weather_region + model, data = EV_data, na.action=na.omit, weights = distance)
summary(eff_h_c_lm)
```
...
#plot(eff_lm)
plot(eff_reg_lm)
plot(eff_h_c_lm, col = c(1,2)[vehicle = "ea66a66d",  ])
```
