...
```{r}
decomp_vkt_auck = vkt[vkt$year >= 2003,3:13]$Auckland %>% 
  ts(frequency = 4, start = min(2003)) %>% 
  decompose("multiplicative")

plot_decomp(decomp_vkt_auck, ylab = "Million km travelled", title = "Decomposition of Auckland VKT Time Series")
```
...
points(x = seq(0,4,0.01), sin_func(seq(0,12,0.03),best_comb[1],best_comb[2],best_comb[3]), type = "l")
```
