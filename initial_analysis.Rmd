...
including model of car in model is worth it, interaction anova says is significant but fit does not really improve and most are not significant. i.e not really worth being concerned about how different car respond to difference in temps
```{r}
...
par(mar = c(4, 4, 2, 4))
plot(as.numeric(mean_HDD_auckland$Month), mean_HDD_auckland$kwh, type = 'b', xaxt = "n",
     ylab = "average kWh usage", xlab = "Month", main = "Auckland EV average kWh usage and Monthly Heating Days by Month 2017-2019")
axis(1, labels = as.character(mean_HDD_auckland$Month), at = as.numeric(mean_HDD_auckland$Month))
...
while it is clear that efficiency decreases in the winter months going by fuel usage data total power used should be less in the winter as there is more driving in the summer.
is this cause petrol fuel efficiency is lower in summer, current EV drivers are different (early adopters or EV commute with 2nd petrol road trip car), or a regional thing?
...
petrol fuel efficiency is lower in summer or current EV drivers are different (early adopters or EV commute with 2nd petrol road trip car)?