...
```{r}
anova(monthly_eff_lm, monthly_eff_qm, monthly_eff_qm_log)
```
...
```{r}
par(mar = c(5, 4, 2, 1))
plot(monthly_data$m, monthly_data$median_kwh, type = 'l', xaxt = "n", xlab = "", ylab = "Mean power usage of EVs", main = "Time series of EV kWh Useage")
...
```{r}
plot(card_fuel$month[card_fuel$year == 2003],card_fuel$fuel_purchased[card_fuel$year == 2003],
...
money spent shows a very clear upwards trend towards December (trips). with a valley in the winter. interesting that jan is lowest point. maybe new years resolutions to bike/walk or no school but not as many trips as dec?
